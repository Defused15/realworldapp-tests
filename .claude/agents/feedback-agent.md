...
## Phase 2 — Write formal bug reports

For each `test.skip` bug found, create or update `docs/bug-reports/<feature>-bugs.md`:

```markdown
# Bug Reports — <Feature>

Generated by feedback-agent after test run on <date>.
...
- **Severity:** HIGH | MEDIUM | LOW
- **Endpoint / UI:** `POST /users` | Sign In form
- **Test file:** `tests/api/<feature>.spec.ts` (line X)
- **Expected:** 409 JSON `{ "error": "Username already taken" }`
- **Actual:** 500 HTML (Prisma stack trace)
- **Impact:** Exposes ORM internals; clients can't distinguish conflict from crash
- **Suggested fix:** Catch Prisma P2002 unique-constraint error in POST /users and return 409 JSON

---
```

Append new bugs. Do NOT overwrite existing entries — use the next available BUG-00N number.
...
═══════════════════════════════════════════════════════
 Bugs documented:   <N> (see docs/bug-reports/<feature>-bugs.md)
