...
test.describe('Signup', () => {
  ...
  test.describe('Registration Form', () => {
    ...
    test.describe('Happy Path', () => {
      ...
      test('no inline errors are shown on initial page load @smoke', async ({
        ...
        await expect(signupPage.validationErrorMessages).toHaveCount(0);
      });
    ...
    test.describe('Accessibility', () => {
      ...
      test('all 5 inputs are reachable by label @a11y', async ({
        signupPage,
      }) => {
  ...
  test.describe('Have an account? Sign In Link', () => {
    // ─── Happy Path ──────────────────────────────────────────────────────────
    test.describe('Happy Path', () => {
      test('link is visible on the signup page @smoke', async ({
        signupPage,
      }) => {
        await expect(signupPage.signInLink).toBeVisible();
      });

      test('link has href="/signin" @smoke', async ({signupPage}) => {
        await expect(signupPage.signInLink).toHaveAttribute('href', '/signin');
      });

      test('navigating to /signin loads the signin form @smoke', async ({
        signupPage,
        page,
      }) => {
        await expect(signupPage.signInLink).toHaveAttribute('href', '/signin');
        ...
        await expect(page).toHaveURL(/signin/);
        await expect(new SigninPage(page).submitButton).toBeVisible();
      });
    });
    ...
    test.describe('Accessibility', () => {
      test('link has accessible text (not empty) @a11y', async ({
        ...
        const text = await signupPage.signInLink.textContent();
