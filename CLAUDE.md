...
- **[signin]** BUG-003/BUG-004 (bcrypt hash leak in `user.password`) were **re-verified FIXED on 2026-06-16** — `POST /users` and `POST /login` no longer return a `password` field. Tests must assert `not.toHaveProperty('password')`; never re-add `password` to a response allowlist.
- **[signin]** BUG-001 (missing required fields) and BUG-002 (duplicate username) were **re-verified FIXED on 2026-06-16** — `POST /users` now returns **422 JSON** (`{errors:[...]}`) for missing fields and **409** for a duplicate username (was 500 HTML). Assert these exact codes; do not accept 500.
...
- **[all]** **`@playwright/test`/`playwright` are PINNED to exact `1.60.0` (no caret) — do NOT bump to `1.61.0`.** Playwright 1.61.0's synchronous ESM loader (`esmLoaderSync.ts`, via Node's `module.registerHooks`) assumes `context.conditions` is an array, but on **Node 22.15.x** it is not — every spec that imports a local `.ts` module dies at import with `TypeError: context.conditions?.includes is not a function` and **0 tests run** (the failure is silent in `--list` too). Vitest/tsx are unaffected, so unit tests stay green and hide it. Re-pin only once a Playwright release fixes this or CI's Node moves off 22.15.x.
...
- **[home]** BUG-HOME-001 (date-filter params crashed `GET /transactions/public` with 500) was **re-verified FIXED on 2026-06-16** — date-filter params now return **200**. Also BUG-TXN-API-001 (GET non-existent transaction returned 200) is fixed → now **404**, and BUG-API-FUZZ-006 (PATCH to a taken username) no longer 500s.
