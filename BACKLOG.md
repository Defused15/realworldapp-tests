...
| 🟡 **Contract/property testing (Schemathesis)** — EN CURSO: spec OpenAPI escrito a mano (`docs/api/openapi.yaml`, la app no lo tenía) + runner (`npm run test:schemathesis`). El fuzzer ya encontró **7 server errors (500)** de robustez (query param desconocido en /transactions/public, FK violations en POST /transactions y /comments, items no-array en /notifications/bulk, etc.). Falta: fixear esos 7 y volverlo gate en CI. | Medio   | Medio                          |
