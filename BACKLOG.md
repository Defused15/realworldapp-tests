...
| ~~**Contract/property testing (Schemathesis)**~~ — ✅ HECHO (2026-06-16): es **gate de regresión** en `pipeline.yml` (ya no `continue-on-error`). Los bugs conocidos del app se excluyen por `operationId` en `tests/scripts/schemathesis.sh` (mapeados a `BUG-API-FUZZ-00x` en `bugs.yml`), así el job queda **verde con la deuda existente y rojo solo ante un 500 NUEVO**. El propio gate destapó **BUG-API-FUZZ-009** (PATCH /users/{id} con body mal tipado → 500, además corrompe al seed user). Los 500 viven en el repo del app (REGLA #1: no los podemos fixear nosotros) → siguen como issues. Re-incluir un id en `EXCLUDED_OPS` cuando el app lo arregle.                                                                                                                                                                                                                                                                                                                                                                                                                             | Medio   | Medio                         |
| ✅/🟡 **Ephemeral environments por PR** — RESUELTO en su valor núcleo (2026-06-16): el **aislamiento ephemeral ya existe POR RUN** — `setup-app`/`app.ci.yml` levanta un app+Postgres aislado, auto-seedeado (`SEED_ON_BOOT`) y desechable desde la imagen GHCR, y la suite destructiva completa corre ahí en push. Eso cubre el motivo del item (env aislado/seedable/desechable para tests destructivos sin contaminar nada). **Extra hecho:** Railway `rwa-staging` (fijo/compartido, same-origin `rwa-app-production-9842.up.railway.app`) + monitoreo sintético **read-only**: project `staging`, `tests/staging/smoke.spec.ts` (`@staging`), `npm run test:staging:smoke`, workflow `staging-smoke.yml` (cron 6h + manual). **No perseguido:** preview-URL por PR en Railway — `rwa-app` es GitHub-connected, así que `railway env create --duplicate` no lo despliega (copia config + Postgres, pero "No deployment found" para rwa-app). El camino nativo sería el toggle **PR Environments** de Railway en el servicio del app (ligado al repo del APP → fuera de alcance por REGLA #1). | Alto    | Alto (Railway PR Envs nativo) |
...
| ~~**SonarQube / coverage gates**~~ — ❌ DESCARTADO: el objeto de QA es la app, no auditar el código de QA ("QA del QA"). SAST sobre la app violaría black-box (REGLA #1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Medio   | Medio                         |
| ~~**semantic-release + conventional commits + CHANGELOG**~~ — ✅ HECHO: `release.yml` (workflow_run tras pipeline verde) + `.releaserc.json`, versión/CHANGELOG/GitHub Release auto desde Conventional Commits                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Medio   | Bajo                          |
| ~~**Notificaciones** (Slack/PagerDuty on failure)~~ — ❌ DESCARTADO (decisión del usuario)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Bajo    | Bajo                          |
| ~~**Chaos/resilience testing**~~ — ✅ HECHO (2026-06-16): `chaos/` con harness black-box (Pumba + `docker stop`). Experimentos: **db-outage** (assert fail-fast + auto-recovery) y **db-latency** (assert disponibilidad bajo +100ms). `npm run chaos`; job non-gating en `nightly.yml`. Hallazgo: la app falla rápido y se recupera sola, pero **amplifica la latencia de DB ~30×** (+100ms → 2-3.7s en `/transactions/public`, patrón N+1).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Bajo    | Alto                          |
| ~~**devcontainer / Dockerized test runner**~~ — ✅ HECHO: `.devcontainer/devcontainer.json` (Node 20 + docker-in-docker + gh CLI + Playwright + extensiones), Codespaces-ready                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Medio   | Medio                         |
